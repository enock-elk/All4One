// Shared Trello list polling (main thread + module worker).
...
export function shouldIgnore(cardName) {
...
export function createTrelloWatcher(postMessage) {
    let intervalId = null;
    let listStates = {};
    let apiKey = '';
    let token = '';
            ...
            try {
                ...
                for (const result of results) {
                    if (!result.success) {
                        ...
                        postMessage({
                            type: 'log',
                            msg: `Sync error on ${result.target.name}: ${result.error.message}`,
                            isError: true,
                        });
                        continue;
                    }
