...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const boardPip = createBoardPip({
        ...
        onClose: () => localStorage.setItem(PREF_FLOAT_POPUP, 'false'),
    });
    ...
    function syncBoardPip(buckets) {
        latestBuckets = buckets || [];
        if (boardPip.isOpen()) boardPip.render(latestBuckets);
    }

    async function showBoardPip() {
        const result = await boardPip.open(latestBuckets);
        if (result === 'blocked') {
            showAppToast('Popup blocked — allow pop-ups for this site, then try again.');
            return;
        }
        localStorage.setItem(PREF_FLOAT_POPUP, 'true');
    }

    function hideBoardPip() {
        boardPip.close();
    }

    function promptFloatPopup() {
        showActionToast({
            message: 'Open a floating Trello board window? It stays on top and can be moved outside All4One (like YouTube PiP).',
            primaryLabel: 'Open board',
            secondaryLabel: 'Not now',
            onPrimary: () => { void showBoardPip(); },
            onSecondary: () => {
                localStorage.setItem(PREF_FLOAT_POPUP, 'false');
                showAppToast('Re-open anytime from the Board button on Trello Watcher.');
            },
        });
    }

    btnFloatOpen?.addEventListener('click', () => { void showBoardPip(); });
    ...
    function onWatcherEvent(data) {
        switch (data.type) {
            ...
            case 'stats':
                ...
                if (data.buckets) {
                    renderBucketGrid(data.buckets);
                    syncBoardPip(data.buckets);
                }
                break;
            case 'auth_fail':
